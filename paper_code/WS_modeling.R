...
scoringFunction <- function(eta, max_depth, min_child_weight, subsample, colsample_bytree, gamma,
  ...
  ## experiment
  # dtrain <- dtrain[sample(1:nrow(dtrain), 100),]
  # dtest <- dtest[sample(1:nrow(dtest), 50),]
...
scoringFunction <- function(x, final_model = T) {
  # outer_i, cv_folds, data_process, option are defined outside the scoringFunction
  
  c(eta, max_depth, min_child_weight, subsample, colsample_bytree, gamma, m, l, n, account_cum_rain, account_season) %<-%
    x
  ...
  dtrain <- dtrain[sample(1:nrow(dtrain), 100),]
  dtest <- dtest[sample(1:nrow(dtest), 50),]
  val <- lapply(1:5, function(x) 1:20 + (x-1)*20)
...
map_fn <- function(x){
  ...
  integer_col <- c(2,3,7:11)
  x[integer_col] <- round(x[integer_col])
  x
}
...
outDEoptim <-
  DEoptim(
    scoringFunction,
    lower = bounds %>% sapply(function(x) x[1]) %>% as.vector(),
    upper = bounds %>% sapply(function(x) x[2]) %>% as.vector(),
    DEoptim.control(
      NP = 5,
      itermax = 10,
      F = 0.8,
